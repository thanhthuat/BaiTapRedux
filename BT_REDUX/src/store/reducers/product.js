...
const reducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case "ADD_PRODUCT_TO_CARD": {
      ...
      const foundIndex = cloneCard.findIndex(
        (item) => item.product.id === action.payload.id
      );
      if (foundIndex === -1) {
        const cardItem = { product: action.payload, quantity: 1 };
        cloneCard.push(cardItem);
      } else {
        cloneCard[foundIndex].quantity += 1;
      }
    ...
    case "INCREASE_PRODUCT": {
      ...
      const foundIndex = cloneCard.findIndex((item) => {
        console.log(item.id);
    ...
    case "DECREASE_PRODUCT": {
      ...
      if (cloneCard[foundIndex].quantity > 1) {
        cloneCard[foundIndex].quantity -= 1;
      }
      state.card = cloneCard;
      return { ...state };
